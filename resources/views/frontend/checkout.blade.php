        ...
        <form name="checkout" method="post" class="checkout woocommerce-checkout" action="{{route('checkout.insert')}}">
            ...
            <div class="col2-set" id="customer_details">
                <div class="coll-1">
                    ...
                    <p class="form-row form-row form-row-first validate-required" id="billing_first_name_field">
                        ...
                        <input type="text" class="input-text " name="name" id="name" autocomplete="given-name" value="{{Auth::guard('customerlogin')->user()->name}}" />
                    </p>
                    <p class="form-row form-row form-row-last validate-required validate-email" id="billing_email_field">
                        <label for="billing_email" class="">Email Address <abbr class="required" title="required">*</abbr></label>
                        <input type="email" class="input-text " name="email" id="email" autocomplete="email" value="{{Auth::guard('customerlogin')->user()->email}}" />
                    </p>
                    <div class="clear"></div>
                    <p class="form-row form-row form-row-first" id="billing_company_field">
                        <label for="billing_company" class="">Company Name</label>
                        <input type="text" class="input-text " name="company" id="company" autocomplete="organization" value="" />
                    </p>
                    ...
                    <p class="form-row form-row form-row-first address-field update_totals_on_change validate-required" id="billing_country_field">
                        <select name="country_id" autocomplete="country" class="country_to_state country_select" id="country_id">
            ...
            <div id="order_review" class="woocommerce-checkout-review-order">
                <table class="shop_table woocommerce-checkout-review-order-table">
                    ...
                    <tr class="cart-subtotal">
                        <th>Discount</th>
                        <td><span class="woocommerce-Price-amount amount" id="discount">{{session('discount')}}</span>
                        </td>
                    </tr>
                    <tr class="shipping">
                        ...
                        <input type="radio" class="delivery_btn" name="del_charge" value="60">Dhaka
                        <br>
                        <input type="radio" class="delivery_btn"  name="del_charge" value="100">Out Side Dhaka
                        </td>
                    </tr>
                    <tr class="order-total">
                        <th>Total</th>
                        <td><strong><span class="woocommerce-Price-amount amount" id="grand_total">{{$sub_total - session('discount')}}</span></strong> </td>
                    </tr>
                </table>
...
<script>
    $('.delivery_btn').click(function(){
        var del_charge = $(this).val();
        var sub_total = $('#sub_total').html();
        var discount = $('#discount').html();
        var total = sub_total - discount + (parseInt(del_charge));
